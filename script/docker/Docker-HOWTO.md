...
依赖：docker compose v2，删除`name: future-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name future-maven-repo

docker run -it --rm --name future-maven \
    -v future-maven-repo:/root/.m2 \
