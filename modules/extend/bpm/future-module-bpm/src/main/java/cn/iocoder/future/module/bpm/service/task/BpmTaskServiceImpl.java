...
import cn.iocoder.future.framework.common.util.number.NumberUtils;
...
import cn.iocoder.future.framework.web.core.util.WebFrameworkUtils;
import cn.iocoder.future.module.bpm.controller.admin.definition.vo.model.BpmModelMetaInfoVO;
import cn.iocoder.future.module.bpm.controller.admin.task.vo.task.*;
import cn.iocoder.future.module.bpm.convert.task.BpmTaskConvert;
...
import cn.iocoder.future.module.bpm.enums.definition.*;
import cn.iocoder.future.module.bpm.enums.task.BpmCommentTypeEnum;
import cn.iocoder.future.module.bpm.enums.task.BpmReasonEnum;
import cn.iocoder.future.module.bpm.enums.task.BpmTaskSignTypeEnum;
...
import cn.iocoder.future.module.bpm.service.definition.BpmFormService;
import cn.iocoder.future.module.bpm.service.definition.BpmModelService;
...
import cn.iocoder.future.module.bpm.service.message.dto.BpmMessageSendWhenTaskTimeoutReqDTO;
...
import static cn.iocoder.future.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
//        }
    ...
    public List<UserTask> getUserTaskListByReturn(String id) {
        ...
        // 2.3 过滤：只能退回到已经处理过的节点（排除审批未经过的节点）。相关 issue：https://github.com/YunaiV/future-vue-pro/issues/982
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 future-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void returnTask(Long userId, BpmnModel bpmnModel, Task currentTask, FlowElement targetElement, BpmTaskReturnReqVO reqVO) {
        ...
        //    相关 issue：https://github.com/YunaiV/future-vue-pro/issues/1018
    ...
    public void delegateTask(Long userId, BpmTaskDelegateReqVO reqVO) {
        ...
        // 特殊：如果已经被委派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/future-cloud/issues/ICJ153
    ...
    public void transferTask(Long userId, BpmTaskTransferReqVO reqVO) {
        ...
        // 3.1 设置任务所有人 (owner) 为原任务的处理人 (assignee)
        // 特殊：如果已经被转派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/future-cloud/issues/ICJ153
        if (StrUtil.isEmpty(task.getOwner())) {
            taskService.setOwner(taskId, task.getAssignee());
        }
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://gitee.com/zhijiantianya/future-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
